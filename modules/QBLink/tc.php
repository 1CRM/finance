...
class QBTaxCode extends QBBean {
	...
	function &get_qb_tax_code($server_id, $qb_id) {
		QBTaxCode::load_taxes($server_id);
	...
	function &get_qb_tax_rate($server_id, $qb_id) {
		QBTaxCode::load_taxes($server_id);
	...
	function &from_iah_tax_code($server_id, $iah_id, $tax_rates=null) {
		QBTaxCode::load_taxes($server_id);
		...
		if(isset($map[$iah_id])) {
			$ret = QBTaxCode::get_qb_tax_code($server_id, $map[$iah_id]);
		}
		if($ret && is_array($tax_rates)) {
			// does not check compounding or order of tax rates
			$check = array();
			foreach(QBTaxCode::get_iah_code_rates($iah_id) as $r)
			...
			if(! $ok) {
				$alt_code = QBTaxCode::match_iah_tax_rates($server_id, $tax_rates);
				if($alt_code)
					$ret = QBTaxCode::get_qb_tax_code($server_id, $map[$alt_code]);
				else
					$ret = new QBTaxCode();
			}
		}
	...
	function &from_iah_tax_rate($server_id, $iah_id) {
		...
		$ret = null;
		if(isset($map[$iah_id]))
			$ret = QBTaxCode::get_qb_tax_rate($server_id, $map[$iah_id]);
		return $ret;
	}
	function &to_iah_tax_code($server_id, $qb_id) {
		QBTaxCode::load_taxes($server_id);
	...
	function &standard_qb_tax_code($server_id) {
		$code = QBTaxCode::from_iah_tax_code($server_id, STANDARD_TAXCODE_ID);
		return $code;
	}
	
	function &exempt_qb_tax_code($server_id) {
		$code = QBTaxCode::from_iah_tax_code($server_id, '-99');
		return $code;
	}
	
	// static
	function load_taxes($server_id, $reload=false) {
		$modkey = 'TaxCode';
		$qb_objs = null;
		if(! $reload)
		...
		QBTaxCode::load_tax_rates($server_id, $reload);
		
		$bean = new QBTaxCode();
		$where = ' server_id="'.$bean->db->quote($server_id).'" ';
		$all = $bean->get_full_list('', $where);
		$qb_objs = array();
		$iah_vals = $bean->get_iah_tax_codes();
		$map_qb = array();
		$map_iah = array();
		$none_mapped = true;
		if($all)
	...
	// static
	function load_tax_rates($server_id, $reload=false) {
		$modkey = 'TaxRate';
		$qb_objs = null;
		if(! $reload)
			$qb_objs =& qb_get_cache($server_id, $modkey, 'QB');
		...
		$map_iah = array();
		$bean = new QBTaxCode();
		$iah_vals = $bean->get_iah_tax_rates();

		$edition = QBServer::get_server_edition($server_id);
		if($edition == 'US') {
			$none_mapped = true;
			$all = QBTaxCode::load_us_sales_taxes($server_id);
			if($all)
				foreach($all as $bean) {
					$qb_objs[$bean->qb_id] = $bean;
	...
	function get_iah_code_rates($code_id) {
		static $rates;
		if(! isset($rates))
			$rates = array();
	...
	function &load_us_sales_taxes($server_id) {
		global $db;
		...
		if($step == 'TaxRates') {
			if($edition == 'US') {
				if(isset($_REQUEST['taxrates_map']) && is_array($_REQUEST['taxrates_map'])) {
					...
					QBTaxCode::load_tax_rates($server_id, true);
				}
			}
			else {
				if(isset($_REQUEST['taxrates_map']) && is_array($_REQUEST['taxrates_map'])) {
	...
	function check_sales_tax_enabled($server_id) {
